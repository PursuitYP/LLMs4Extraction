...
def relation_alignment(sentence, head, tail, relation, schema_relations):
    ...
    prompt_prefix_copy = f'''You will read a "sentence" and a "triple" \[head entity, "relation", tail entity\] extracted from that sentence.
Please align the "relation" into one of the following "categories" based on the contextual information of this relation in the sentence.
If you think the relation does not belong to any of the following categories, output \"none\".
Your output can only be one of these categories or \"none\".
'''
...
def process_relations(sentence, relations, entity_labels, schema_relations):
    ...
    for relation_info in relations:
        ...
        if relation_aligned == "none":
            ...
            continue
        # print(head, " @ ", relation, " @ ", tail)
        # print(head, " @ ", relation_aligned, " @ ", tail)
        # print("-" * 100)
...
# rel_pubmed_0508, rel_selected_papers, rel_clinical_trials
# rel_1_10001, rel_230001_240001, rel_380001_390001 rel_490001_500001
# rel_file = "../../projects/openie6/rel_results/rel_pubmed_0508.txt"
rel_file = "../../projects/openie6/predictions.txt"
        ...
        if True:
            ...
            if line == "":
                ...
                if relations != []:
                    ...
                    # """ 在这里根据sentences对relations进行处理, 如实体链接和关系对齐 """
                    # new_relations = process_relations(sentence, relations, entity_labels, schema_relations)
                    # # 新关系结构：[head, head_label, relation_aligned, 0ld_relation, tail, tail_label]
